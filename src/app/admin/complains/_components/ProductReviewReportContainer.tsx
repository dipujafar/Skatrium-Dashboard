...
import { useGetAllProductReportsQuery } from "@/redux/api/productReportApi";
import { useSearchParams } from "next/navigation";
import ProductReportCard from "./ProductReviewReportCard";
...
const ProductReportContainer = () => {
    ...
    const { data, isLoading } = useGetAllProductReportsQuery({ page, limit: pageSize });
    ...
    return (
        <div className="border border-[#FFFFFF33]/[0.2] rounded-2xl py-5 px-4">
            <h4 className="text-[#93A4B0] text-xl font-semibold mb-4">All Product Reports</h4>
            ...
            <div>
                {total > 0 && <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 justify-items-center">
                    {reports.map((report: any) => (
                        <ProductReportCard key={report?._id} report={report} />
                    ))}
                </div>}
            </div>
            <PaginationSection
